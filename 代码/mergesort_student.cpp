...
void disp(Student a[],int n)			//输出a中所有元素
{	int i;
	for (i=0;i<n;i++)
		cout<<a[i].score<<"  ";
	cout<<endl;
...
{	
	...
	Student *tmpa=new Student[(high-low+1)*sizeof(Student)];                   //动态分配空间
...
void MergePass(Student a[],int length,int n)	//一趟二路归并排序
{	int i;
...
void MergeSort(Student a[],int n)			//二路归并算法
{	int length;
